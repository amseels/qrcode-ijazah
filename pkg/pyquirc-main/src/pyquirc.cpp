...
Decoder Decoder_from_image(py::buffer buffer) {
    ...
    if (info.ndim != 2) {
        throw invalid_argument("Image must be 2d uint8 (8-bit grayscale)");
    }
...
Decoder Decoder_from_bytes(py::buffer buffer, size_t width, size_t height) {
    py::buffer_info info = buffer.request();
    if (info.ndim != 1) {
        throw invalid_argument("Image must be a bytes-like object");
    }

    auto expected_size = static_cast<py::ssize_t>(width * height);
    if (info.size != expected_size) {
        throw invalid_argument("Size of buffer does not match image dimensions");
    }
...
py::object Data_payload(const Data& self) {
    return py::bytes(string(self.payload.begin(), self.payload.end()));
}

py::object Data_payload_no_ecc(const Data& self) {
    return py::bytes(string(self.payload_no_ecc.begin(), self.payload_no_ecc.end()));
}

py::object Data_payload_after_ecc(const Data& self) {
    return py::bytes(string(self.payload_after_ecc.begin(), self.payload_after_ecc.end()));
}


vector<vector<bool>> Code_matrix(const qr::quirc_code& self) {
    size_t size = static_cast<size_t>(self.size);
    vector<vector<bool>> matrix(size);
...
PYBIND11_MODULE(quirc, m) {
    ...
    py::class_<qr::quirc_code>(m, "Code")
        .def_property_readonly("corners", [](const qr::quirc_code& self) {
            return vector<qr::quirc_point>(self.corners, self.corners + 4);
        })
    ...
    py::class_<Data>(m, "Data")
        ...
        // .def_readonly("payload_len", &Data::payload_len)
        .def_property_readonly("payload_no_ecc", &Data_payload_no_ecc)
        // .def_readonly("payload_no_ecc_len", &Data::payload_no_ecc_len)
        .def_property_readonly("payload_after_ecc", &Data_payload_after_ecc)
        ;
