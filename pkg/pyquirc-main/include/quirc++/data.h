...
struct Data {
    ...
    // int payload_len = 0;
    std::vector<uint8_t> payload_no_ecc = {};
    // int payload_no_ecc_len = 0;
    std::vector<uint8_t> payload_after_ecc = {};
    // int payload_after_ecc_len = 0;
        ...
        eci(ECI(data.eci))
    {
        ...
        payload_no_ecc = std::vector<uint8_t>(data.payload_no_ecc, data.payload_no_ecc + data.payload_no_ecc_len);
        payload_after_ecc = std::vector<uint8_t>(data.payload_after_ecc, data.payload_after_ecc + data.payload_after_ecc_len);
