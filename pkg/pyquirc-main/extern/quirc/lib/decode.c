					   ...
					   struct datastream *ds)
{
	...
	/*
	* Memodifikasi for dibawah ini dengan memasukan loop tambahan.
	*/
	uint8_t	data_cwx[ver->data_bytes];
	int idx_data_cwx = 0;
	...
	for (i = 0; i < bc; i++) {
		...
		/* Script Modifikasi 
		* memasukan data raw decimal yang ada pada array dst ke data_cwx.
		* data_cwx data array data codeword sebelu di lakukan proses error
		* correction
		*/
		int loop;
		for (loop = 0; loop < ecc->dw; loop++){
			data_cwx[idx_data_cwx] = dst[loop];
			idx_data_cwx += 1;
		}
		/* Selesai */
	...
	/* Modifikasi 
	* Membuat sebuah array dengan panjang array
	* adalah sebanyak data codeword. didapatkan
	* dari nilai idx_data_cwx
	* script ini di buat agar codeword bypass ec
	* dapat di panggil pada demo script.
	*/
	int n = sizeof(data_cwx) / sizeof(data_cwx[0]);
	uint8_t data_codeword[idx_data_cwx];
	for (int jx = 0; jx < idx_data_cwx; jx++){
		data_codeword[jx] = data_cwx[jx];
		data->payload_no_ecc[jx] = data_cwx[jx];
	}
	data->payload_no_ecc_len = idx_data_cwx;
	/* Selesai */

	/* get data padding setelah ecc */
	for (int jy = 0; jy < idx_data_cwx; jy++){
		// printf("%d ",ds->data[jy]);      
		data->payload_after_ecc[jy] = ds->data[jy];
	}
	data->payload_after_ecc_len = idx_data_cwx;
