                                   ...
                                   zbar_image_t *img)
{
    ...
    for (i = 0; i < nqrdata; i++)
        if (!mark[i]) {
            ...
            unsigned char *payload_after_ecc;
            int payload_after_ecc_length;
           ...
           parity alone are too ambiguous.*/
            ...
            for (j = 0; j < sa_size; j++)
                if (sa[j] >= 0) {
                    qrdataj = qrdata + sa[j];
                    version = qrdataj->version;
                    ecc_level = qrdataj->ecc_level;
                    for (k = 0; k < qrdataj->nentries; k++) {
                        ...
                        /* QRCode-Ijazah: #MODIFIED_START_HERE */
                        payload_after_ecc = entry->payload.data.buf_after_ecc;
                        payload_after_ecc_length = entry->payload.data.len_after_ecc;
                        /* #MODIFIED_END_HERE */
        ...
        We already checked that it lies in one of the ranges A...Z, a...z
         when we decoded it.*/
            ...
            if (!err) {
                ...
                /* QRCode-Ijazah: #MODIFIED_START_HERE */
                sa_sym->payload_after_ecc = payload_after_ecc;
                sa_sym->payload_after_ecc_length = payload_after_ecc_length;
                sa_sym->version = version;
                sa_sym->ecc_level = ecc_level;
                sa_sym->eci = eci;
                /* #MODIFIED_END_HERE */
